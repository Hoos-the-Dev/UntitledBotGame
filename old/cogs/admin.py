...
class Admin(commands.Cog):
  ...
  async def kick(self, ctx, member: nextcord.Member, *, reason=None):
    blacklist = 637997826571239454
    if ctx.author.id == blacklist:
      await ctx.reply("why should i. you're blacklisted")
    else:
  ...
  async def purge(self, ctx, amount=11):
      ...
      if amount < 3:
        ...
        return
      elif ctx.author.id == blacklist:
          embed = nextcord.Embed(title="Fuck off.", description=f"if i check my book it says that **{ctx.author.name}** has been blacklisted from using this command.", color=nextcord.Color.red())
          embed.set_footer(text="how did you fuck up that badly to where you got blacklisted.")
          await ctx.reply(embed=embed)
      else: 
  ...
  async def purge_error(self, ctx, error,):
      ...
      await ctx.reply(embed=purgeerror)
